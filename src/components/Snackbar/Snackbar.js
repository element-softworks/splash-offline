...
import { camelCase, pascalCase } from '@helpers/index';
	...
	return (
		<SnackbarContext.Provider value={{ openSnackbar, closeSnackbar }}>
			...
			<CSSTransition
				...
				className={`${styles.snackbarWrapper} ${styles[`snackbarWrapper${pascalCase(position)}`]}`}
				classNames={{
					enter: `${styles.snackbarEnter} ${styles[`snackbarEnter${pascalCase(position)}`]}`,
					enterActive: `${styles.snackbarEnterActive} ${
						styles[`snackbarEnterActive${pascalCase(position)}`]
					}`,
					exitActive: `${styles.snackbarExitActive} ${
						styles[`snackbarExitActive${pascalCase(position)}`]
					}`,
				}}
			>
				...
				<div>
					<div className={styles.snackbar} style={customStyles}>
						{/* Snackbar's text */}
						<div className={styles.snackbarText}>{text}</div>

						{/* Snackbar's close button */}
						<button onClick={closeSnackbar} className={styles.snackbarClose} style={closeCustomStyles}>
							<CloseIcon />
						</button>
					</div>
				</div>
